...
namespace SYSPROHISTORICALTEST2
{
    public partial class frmCustomerQuery : Form
    {
        ...
        private void btnFind_Click(object sender, EventArgs e) //find all customers that meet search criteria, select first one
        {
            ...
            if (checkedListBox1.CheckedItems.Count > 0) //if query filter selected (cust invoices, cust sales orders, etc.)
            {
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e) //selects the customer number of the selected row, and if there is a secondary query chosen, updates datagrid 2 to the selected customer
        {
        ...
        private void dataGridView2_CellClick(object sender, DataGridViewCellEventArgs e) //selects the customer number of the selected row, and if there is a secondary query chosen, updates datagrid 2 to the selected customer
        {
            ...
            getCustomerTertiaryQuery(); //get secondary info for selected customer
        ...
        private void getCustomerSecondaryQuery()
        {
            if (selectedCustomer != "") //get secondary data grid view results
            {
                ...
                switch (comboBox1.SelectedItem) //choose what the secondary query is (all from table x where customer = selected customer)
                {
                    case "Invoices":
                        select = "SELECT * FROM ARINVOICE";
                        break;
                    case "Movements":
                        select = "SELECT * FROM ARSALESMOVE"; //or just movements?
                        break;
                    case "Payments":
                        select = "SELECT * FROM ARINVOICEPAY";
                        break;
                    case "Work in Progress":
                        MessageBox.Show("Work in progress module has never used, not implemented");
                        break;
                    case "RMA":
                        MessageBox.Show("RMA module has never used, not implemented");
                        break;
                    case "Sales Orders":
                        select = "SELECT * FROM SORMASTER";
                        break;
                    case "Quotations":
                        MessageBox.Show("Quotations module has never used, not implemented");
                        break;
                ...
                //NOTE: if the key field isn't the first column the line in the if needs to change
        ...
        private void getCustomerTertiaryQuery() //gets extra details for the secondary queries if they normally have 2 datagridview in syspro
        {
            ...
            if (comboBox1.SelectedItem != null && (comboBox1.SelectedItem.ToString() == "Invoices" || comboBox1.SelectedItem.ToString() == "Sales Orders")) 
            {
                if (selectedSecondaryKey != "") //get tertiary data grid view results
                {
                    ...
                    //get start the select statement for tertiary datagridview
                    switch (comboBox1.SelectedItem) //choose what the secondary query is (all from table x where customer = selected customer)
                    {
                        case "Invoices":
                            select = "SELECT * FROM ARINVOICEPAY ";
